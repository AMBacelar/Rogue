	...
	public void Populate(IntVector2 pos, int monsterDensity, int depth)
	{
		if (Random.Range(0, 3000) < monsterDensity + (depth / 4))
		{
			...
			else
			{
				...
				enemyGO.AddComponent<MoveAction>();
				enemyGO.AddComponent<LinearWeapon>();
				enemyGO.GetComponent<LinearWeapon>().weapomName = "wooden sword";
				enemyGO.GetComponent<LinearWeapon>().range = 1;
