...
public class BoardManager : MonoBehaviour
{
	...
	public void InitAStar(Hex[,] mapIn)
	{
		grid = new RoyT.AStar.Grid(mapIn.GetLength(0), mapIn.GetLength(1), 1.0f);
		foreach (Hex hex in mapIn)
		{
			if (hex.isWalkable)
			{
				grid.SetCellCost(new Position(hex.Q, hex.R), tileTypes[hex.tileType].movementCost);
			}
			else
			{
				grid.BlockCell(new Position(hex.Q, hex.R));
			}
		}
	}
